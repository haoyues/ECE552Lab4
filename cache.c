...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
    ...
    if (cache_probe(cp, (addr + cp->bsize) & ~cp->blk_mask))
    {
        cp->prefetch_hits++;
    }
    else
    {
        ...
        cache_access(cp, Read, (addr + cp->bsize) & ~cp->blk_mask, NULL, cp->bsize, NULL, NULL, NULL, 1);
    }
}
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
	//get the index of the RPT using PC
    //md_addr_t RPTIdx = (get_PC() >> PC_SHIFT) & ((cp->prefetch_type)-1);
    md_addr_t RPTIdx = (get_PC() >> (int)log2((double) sizeof(md_inst_t))) & ((cp->prefetch_type)-1);
    //the tag is the PC
    md_addr_t tag = get_PC();
    RPTEntry entry = cp->RPT[RPTIdx];
    int stride;
    
    //the tag is in the RPT
    if(tag == entry.tag)
    {
    ...
    //the tag is not in the RPT, insert the tag
    else
    {
        entry.tag = tag;
        entry.prev_addr = addr;
    ...
    //prefetch data
    if((cp->RPT[RPTIdx].stride != 0) && (cp->RPT[RPTIdx].state != NO_PRED)) {
        //the block is already in the cache
        if(cache_probe(cp, ((addr + cp->RPT[RPTIdx].stride) & ~cp->blk_mask))) {
            cp->prefetch_hits++;
        }
        else
        {
            cache_access(cp, Read, ((addr + cp->RPT[RPTIdx].stride) & ~cp->blk_mask), NULL, cp->bsize, NULL, NULL, NULL, 1);
        }
    }
}
